function Create(self)
	self.name = "[DZ] Compound Crossbow"
	--empty name is "Empty ".. self.name

	self.recoilrisefactor = 0.01 --radians of muzzle rise to add when a shot is fired
	self.recoilriserecovery = 0.20 --radians per second of muzzle rise recovery
	self.recoilrisemax = 0.95 --coefficient of muzzle rise that's retained per script update, creating a limit of total muzzle rise
	self.recoilrise = 0
	
	self.recoilspreadfactor = 0.025 --radians of variance from (aim + muzzle rise) to add when a shot is fired
	self.recoilspreadrecovery = 0.08 --radians per second of variance recovery
	self.recoilspreadmax = 0.95 --coefficient of variance that's retained per script update, creating a limit of total muzzle rise
	self.recoilspread = 0
	
	self.recoilsharplengthfactor = 30 --px
	self.recoilsharplengthrecovery = 200 --px/s
	self.recoilsharplengthoriginal = -1 --original aim distance
	
	self.recoiltimer = Timer()
	
	
	self.reloadtime = 1000 --ms
	self.reloadtiltangle = 0.6 --radians
	self.reloadtiltspeed = 3 --radians per second tiltspeed
	self.reloadendspread = 0.1 --radians of variance from having just finished the reload
	...
	self.ammobox = "Metal Bolt"
	self.roundsperbox = 2
end
function Update(self)
	
	--make sure the root ID is valid and the object it's referencing is valid.
	if self.RootID ~= 255 then
		self.y = MovableMan:GetMOFromID(self.RootID)
		if self.y then
			if self.y:IsActor() then --check if it's an actor and reference it as such if so
				...
				elseif self.recoilrise > 0 then --if recoil's above 0, there's still recoil to recover from
					self.recoilrise = (self.recoilrise - (self.recoilriserecovery * self.recoiltimer.ElapsedSimTimeS)) * self.recoilrisemax
				...
				elseif self.recoilspread > 0 then
					self.recoilspread = (self.recoilspread - (self.recoilspreadrecovery * self.recoiltimer.ElapsedSimTimeS)) * self.recoilspreadmax
				elseif self.recoilspread < 0 then
					self.recoilspread = (self.recoilspread + (self.recoilspreadrecovery * self.recoiltimer.ElapsedSimTimeS)) * self.recoilspreadmax
				end
				--SIGHTING recovery
