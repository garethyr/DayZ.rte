...
(string.find(particle.PresetName,"Flame") ~= nil) or (string.find(particle.PresetName,"Napalm") ~= nil) or (string.find(particle.PresetName,"Spark") ~= nil) or (string.find(particle.PresetName,"Blast") ~= nil) or(string.find(particle.PresetName,"Muzzle Flash") ~= nil) )  and self.Timer:IsPastSimMS(1700) then 
			if SceneMan:ShortestDistance(self.Pos,particle.Pos,self.mapwrapx).Magnitude < 75 then
				self.ToDelete = true;
				...
				flame.Pos = self.Pos;
				flame.Vel = Vector((math.random()*12)+12,0):RadRotate(math.random()*6.3728);
				--flame.Vel = self.Vel*9;
